...
function get20s(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let birthYear = Number(arr[i].years.substring(0, 4));
    if (birthYear > 1800 && birthYear < 1900) {
      get20sArr.push(arr[i].name);
    }
  }
  return get20sArr;
}
...
addArtist(
  ...
  'Web Design',
  'American',
  'An aspiring web developer that likes cats and playing video games.'
);
...
console.log(checkArtist('Barbara Moore', artists)); // returns true because of previous task
console.log(checkArtist('Sydney the Cat', artists)); // returns false
